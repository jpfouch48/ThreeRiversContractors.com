 ...
 * @var array
 */
$args = (array) apply_filters( 'trc_wc_pagination_args', array(
	'type'      => 'plain',
	'end_size'  => 1,
	'mid_size'  => 2,
...
<nav class="navigation pagination woocommerce-pagination">
	...
	<h2 class="screen-reader-text"><?php printf( /* translators: post type singular label */ esc_html__( '%s navigation', 'trc' ), esc_html( $post_type_label ) ); ?></h2>

	<div class="paging-nav-text"><?php printf( /* translators: 1. current page number, 2. total number of pages */ esc_html__( 'Page %1$d of %2$d', 'trc' ), max( 1, get_query_var( 'paged' ) ), absint( $wp_query->max_num_pages ) ); // xss ok. ?></div>
